...
# ——— SESSION RESTORE (MUST BE BEFORE CLIENT CREATION) ———
SESSION_NAME = "session_auto_redeemer"
...
def restore_session():
    ...
    if session_b64:
        ...
        print("⚠️ No SESSION_B64 found - will try fresh login.")

restore_session()  # ← runs BEFORE client is created
...
def load_numbers_to_memory():
    ...
    try:
        ...
        # ── 2. Try numbers.txt in same folder (Railway with file) ──
        ...
        # ── 3. Fallback to hardcoded path (local PC) ──
        ...
        print("❌ No numbers source found! Add numbers.txt or PHONE_NUMBERS env var.")
...
async def submit_number(idx, mobile, gift_code, stats, retry_queue, semaphore):
    ...
    try:
        async with semaphore:
            async with GLOBAL_SESSION.post(
                SUBMIT_URL, data=payload, headers=headers, timeout=10
            ) as response:
...
async def process_real_code(gift_code: str):
    ...
    print(f"\n{'='*55}")
    print(f"🚀 INITIATING GATLING GUN SEQUENCE: {gift_code}")
    print(f"📦 Total Numbers Loaded : {len(numbers)}")
    print(f"⏱️ Firing interval      : {FIRE_RATE*1000:.1f}ms per request")
    print(f"{'='*55}\n")
    ...
    print(f"\n🏆 ALL DONE in {elapsed:.2f}s | Total Success: {stats['success']}")
    log(
...
async def resolve_and_join_channel(invite_url: str):
    try:
        ...
        if not hash_match:
            print(f"❌ Cannot extract invite hash from: {invite_url}")
            return None
        ...
        try:
            ...
            if "already" in err_str or "user_already" in err_str:
                print(f"ℹ️ Already a member of: {invite_url}")
            else:
                print(f"⚠️ Join attempt result: {join_err}")

        try:
            entity = await client.get_entity(invite_url)
            if entity:
                print(f"✅ Resolved: {getattr(entity, 'title', entity.id)} (ID: {entity.id})")
                return entity
        except Exception as get_err:
            print(f"❌ get_entity failed for {invite_url}: {get_err}")

        return None

    except Exception as e:
        print(f"❌ resolve_and_join_channel error for {invite_url}: {e}")
        return None
...
async def main():
    ...
    # Register separate handler per channel using real ID
    for entity in RESOLVED_CHANNELS:
        client.add_event_handler(
            on_new_message,
            events.NewMessage(chats=[entity.id])
        )
        print(f"🎯 Listening to: {getattr(entity, 'title', entity.id)}")

    print(f"\n👂 Locked on {len(RESOLVED_CHANNELS)} channel(s). Waiting for gift codes...\n")
