    ...
    try:
        ...
        for path in possible_paths:
            ...
            if path.exists():
                ...
                if numbers:
                    PRELOADED_NUMBERS = numbers
                    print(f"✅ Pre-loaded {len(PRELOADED_NUMBERS)} numbers from {path}")
                    return
                else:
...
async def process_real_code(gift_code: str):
    ...
    print(f"📦 Numbers loaded  : {len(numbers)}")
...
async def on_new_message(event):
    ...
    if not codes:
        print("ℹ️ No gift code found in message.")
        return
    ...
    if len(codes) > 1:
        print("🕵️ Multiple codes found - probing all...")
        ...
        for code, is_real, is_empty, msg in results:
            if is_real and is_empty:
                ...
                real_code = code
                print(f"🎯 ACTIVE CODE FOUND: {real_code} | {msg}")
                break
            else:
                print(f"❌ Fake code: {code} | {msg}")
        ...
        if is_real and not is_empty:
            print(f"🎯 Code is ACTIVE: {codes[0]} | {msg}")
            await process_real_code(codes[0])
        elif is_real and is_empty:
            print(f"🗑️ Code is EMPTY: {codes[0]} | {msg}")
        else:
...
async def main():
    ...
    try:
        ...
        print("⚠️ Server ping failed but continuing...")
    ...
    if not RESOLVED_CHANNELS:
        print("❌ No channels resolved! Check your invite links.")
    ...
    print(f"\n{'='*55}")
    print(f"👂 LOCKED ON {len(RESOLVED_CHANNELS)} CHANNEL(S)")
    print(f"📦 Numbers ready: {len(PRELOADED_NUMBERS)}")
    print(f"{'='*55}\n")
